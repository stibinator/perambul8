...
function setLegKeyframes() {
    ...
    for (var theLeg = 0; theLeg < selectedLegs.length; theLeg++) {
        if (selectedLegs[theLeg].comment != "perambul8_Body") {
            ...
            theKFs = getSelectedPosAndRotationKeyframes(legControl);
...
function setBody() {
    bodyLayer = app.project.activeItem.selectedLayers;
...
function createControl(legLayer, bodyLayer){
    //walk hierarchy to see if the leg is below the bodyLayer
    walkHierarchy(legLayer, bodyLayer);
...
function walkHierarchy(theLayer, ancestor){
    var oldestAncestor = theLayer;
    if (theLayer.parent !== null) {
        oldestAncestor = walkHierarchy(theLayer.parent, ancestor);
    } else {
        return oldestAncestor;
    }
}

function getSelectedPosAndRotationKeyframes(legLayer){
    //get the selected keyframes from a leg layer to define the forward step
